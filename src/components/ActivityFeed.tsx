...
export default function ActivityFeed({ workouts, users, currentUser }: ActivityFeedProps) {
  ...
  // Helper to get cached comments from localStorage
  const getCachedComments = (): { [workoutId: string]: FeedCommentDoc[] } => {
    try {
      const saved = localStorage.getItem('gpo_health_comments');
  ...
  const saveCachedComments = (data: { [workoutId: string]: FeedCommentDoc[] }) => {
    try {
      localStorage.setItem('gpo_health_comments', JSON.stringify(data));
    } catch (err) {
      console.error('Error saving comments to localStorage:', err);
    }
  };

  // Comments state initialized from localStorage cache
  const [comments, setComments] = useState<{ [workoutId: string]: FeedCommentDoc[] }>(getCachedComments());
  ...
  useEffect(() => {
    ...
    const unsubscribeComments = dbSubscribeComments((rawComments) => {
      const commentMap: { [workoutId: string]: FeedCommentDoc[] } = getCachedComments();

      // Merge Firestore comments into map
      rawComments.forEach((c) => {
        if (!commentMap[c.workoutId]) {
          commentMap[c.workoutId] = [];
        }
        // Deduplicate by id
        const exists = commentMap[c.workoutId].some((item) => item.id === c.id);
        if (!exists) {
          commentMap[c.workoutId].push(c);
        } else {
          // Update existing content
          commentMap[c.workoutId] = commentMap[c.workoutId].map((item) => (item.id === c.id ? c : item));
        }
      });
  ...
  const handleToggleReaction = async (workoutId: string, type: 'hearts' | 'highFives' | 'fires') => {
    ...
    setReactions((prev) => {
      const current = prev[workoutId] || { hearts: 0, highFives: 0, fires: 0, myReactions: [] };
      const newMyReactions = newActiveState
        ? [...current.myReactions, type]
        : current.myReactions.filter((t) => t !== type);
      const newCount = Math.max(0, current[type] + (newActiveState ? 1 : -1));
  ...
  const handleAddComment = async (workoutId: string) => {
    ...
    setComments((prev) => {
      ...
      const updatedList = [...existing, newComment].sort(
        (a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
      );
      const newMap = { ...prev, [workoutId]: updatedList };
      saveCachedComments(newMap);
      return newMap;
    });
    ...
    try {
      ...
      console.log('Successfully saved comment to Firestore:', newComment);
    } catch (e) {
