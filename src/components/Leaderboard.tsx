  ...
  return (
    <div className="sb-card p-4 sm:p-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-sb-ceramic pb-5 mb-5">
        ...
        {/* Search Input & LINE Notify Trigger */}
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          {isLineAdmin && (
            <button
              ...
              className="px-3.5 py-2.5 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-xs flex items-center gap-1.5 transition-all shrink-0 active:scale-95"
              title="ส่งตารางอันดับเข้ากลุ่ม LINE"
            >
              <MessageSquare className="w-4 h-4 text-emerald-100 fill-emerald-100/30" />
              <span className="hidden sm:inline">ส่งเข้ากลุ่ม LINE</span>
      ...
      <LineIntegrationModal
        ...
        stats={{
          totalSteps: entries.reduce((sum, e) => sum + e.totalSteps, 0),
          totalCalories: entries.reduce((sum, e) => sum + e.totalCalories, 0),
          totalDurationMinutes: entries.reduce((sum, e) => sum + (e.totalDurationMinutes || 0), 0),
          totalWorkouts: entries.reduce((sum, e) => sum + e.totalWorkouts, 0),
          byActivity: {},
        }}
      />
    </div>
